...
<body class="font-muli theme-cyan ">
	...
	<script type="text/javascript">
		$(document).ready(function() {
			...
			$(document).on('click', '.dt-edit-rol', function(evt){
				$this = $(this);
				var dtRow = $this.parents('tr');

				$.ajax({
					url: 'controller/edit_rol_ui.php', 
					type: 'POST',
					data: 'gRI='+dtRow.attr('id'),
					success: function (response) {
						$('.modal-body-edit-rol').html(response);
					}
				});
			...
			$(document).on('click', '.dt-remove-rol', function(evt){
				$this = $(this);
				var dtRow = $this.parents('tr');

				$.ajax({
					url: 'controller/remove_rol_ui.php.', 
					type: 'POST',
					data: 'gRI='+dtRow.attr('id'),
					success: function (response) {
						$('.modal-body-remove-rol').html(response);
					}
				});

				$('#modalRemoveRol').modal('show');
			});


			$(document).on('click', '.dt-add-module-to-role', function(evt){
				$this = $(this);
				var dtRow = $this.parents('tr');

				$.ajax({
					url: 'controller/add_module_to_rol_ui.php.', 
					type: 'POST',
					data: 'gRI='+dtRow.attr('id'),
					success: function (response) {
						$('.modal-body-add-module-to-rol').html(response);
					}
				});

				$('#modalAddModuleToRol').modal('show');
			});
