...
<head>
	...
	<?php echo $physical_maintenance_active = 'active'; ?>
</head>
<body class="font-muli theme-cyan ">
	...
	<div id="main_content">
		...
		<div class="page">
				...
				<div class="container-fluid">
					<div class="d-flex justify-content-between align-items-center ">
						<div class="header-action">
							<h1 class="page-title">Planta f&iacute;sica</h1>
							<ol class="breadcrumb page-breadcrumb">
								<li class="breadcrumb-item"><a href="module.php">Planta f&iacute;sica</a></li>
							</ol>
						</div>
					</div>
				</div>
			...
			<div class="section-body mt-4">
				<div class="container-fluid">
					<?php include 'planta-fisica-info.php';?>
				</div>
			</div>
			<div class="section-body">
	...
	<script type="text/javascript">
		$(document).ready(function() {
			...
			$(document).on('click', '.dt-edit-physical-maintenance', function(evt){
				$this = $(this);
				var dtRow = $this.parents('tr');

				$.ajax({
					url: 'controller/edit_physical_maintenance_ui.php', 
					type: 'POST',
					data: 'gPMI='+dtRow.attr('id'),
					success: function (response) {
						$('.modal-body-edit-physical-maintenance').html(response);
					}
				});

				$('#modalEditPhysicalMaintenance').modal('show');
			});


			$(document).on('click', '.dt-remove-physical-maintenance', function(evt){
				$this = $(this);
				var dtRow = $this.parents('tr');

				$.ajax({
					url: 'controller/remove_physical_maintenance_ui.php.', 
					type: 'POST',
					data: 'gPMI='+dtRow.attr('id'),
					success: function (response) {
						$('.modal-body-remove-physical-maintenance').html(response);
					}
				});

				$('#modalRemovePhysicalMaintenance').modal('show');
			});


			$('.btn-add-level-entity').each(function(){
				var btns = $(this);
				btns.on('click', function(){
					var bid = $(this).data('bid'),
					bdesc = $(this).data('bdesc');
					$('#buildingLevelSelected').html(`<option value="">Seleccione</option> <option selected value="${bid}">${bdesc}</option>`);
				})
			})


			$('.btn-add-class-room').each(function(){
				var btns = $(this);
				btns.on('click', function(){
					var bid = $(this).data('bid'),
					nid = $(this).data('nid');
