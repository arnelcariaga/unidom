...
<body class="font-muli theme-cyan ">
					...
					<div class="d-flex justify-content-between align-items-center ">
						<div class="header-action">
							...
							<ol class="breadcrumb page-breadcrumb">
								...
								<li class="breadcrumb-item active" aria-current="page">Empleados de la instituci&oacute;n</li>
							</ol>
						</div>
					</div>
				</div>
	...
	<script type="text/javascript">
		...
		$('#tableUsers').DataTable({
			...
			$(document).on('click', '.dt-edit-user', function(evt){
				$this = $(this);
				var dtRow = $this.parents('tr');

				$.ajax({
					url: 'controller/edit_user_ui.php', 
					type: 'POST',
					data: 'gUI='+dtRow.attr('id'),
					success: function (response) {
						$('.modal-body-edit-user').html(response);
					}
				});
			...
			$(document).on('click', '.dt-remove-user', function(evt){
				$this = $(this);
				var dtRow = $this.parents('tr');

				$.ajax({
					url: 'controller/remove_user_ui.php', 
					type: 'POST',
					data: 'gUI='+dtRow.attr('id'),
					success: function (response) {
						$('.modal-body-remove-user').html(response);
					}
				});

				$('#modalRemoveUsers').modal('show');
			});
