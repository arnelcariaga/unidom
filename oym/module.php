...
<body class="font-muli theme-cyan ">
				...
				<div class="container-fluid">
					<div class="d-flex justify-content-between align-items-center ">
						<div class="header-action">
							<h1 class="page-title">M&oacute;dulos</h1>
							<ol class="breadcrumb page-breadcrumb">
								<li class="breadcrumb-item"><a href="module.php">M&oacute;dulos</a></li>
							</ol>
						</div>
					</div>
				</div>
	...
	<script type="text/javascript">
		$(document).ready(function() {
			...
			$(document).on('click', '.dt-edit-module', function(evt){
				$this = $(this);
				var dtRow = $this.parents('tr');

				$.ajax({
					url: 'controller/edit_module_ui.php', 
					type: 'POST',
					data: 'gMI='+dtRow.attr('id'),
					success: function (response) {
						$('.modal-body-edit-module').html(response);
					}
				});

				$('#modalEditModules').modal('show');
			});


			$(document).on('click', '.dt-remove-module', function(evt){
				$this = $(this);
				var dtRow = $this.parents('tr');

				$.ajax({
					url: 'controller/remove_module_ui.php.', 
					type: 'POST',
					data: 'gMI='+dtRow.attr('id'),
					success: function (response) {
						$('.modal-body-remove-module').html(response);
					}
				});

				$('#modalRemoveModules').modal('show');
			});
